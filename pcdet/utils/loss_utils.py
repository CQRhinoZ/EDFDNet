...
def _reg_loss_2(regr, gt_regr, mask, weight):
    """
    Refer to https://github.com/tianweiy/CenterPoint
    L1 regression loss
    Args:
...
class RegLossSparse2(nn.Module):
    """
    Refer to https://github.com/tianweiy/CenterPoint
    """

    def __init__(self):
        super(RegLossSparse2, self).__init__()

    def forward(self, output, mask, ind=None, target=None, batch_index=None, weights=None):
        """
        Args:
            output: (N x dim)
            mask: (batch x max_objects)
        ...
        loss = _reg_loss_2(pred, target, mask, weights)
        return loss

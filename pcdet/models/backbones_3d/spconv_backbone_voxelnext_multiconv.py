...
class VoxelResBackBone8xVoxelNeXtMulConv(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.conv2_2 = spconv.SparseSequential(
            block(channels[0], channels[1], spconv_kernel_sizes[0], norm_fn=norm_fn, stride=2,
                  padding=int(spconv_kernel_sizes[0] // 2), indice_key='spconv2', conv_type='spconv'),
            block(channels[1], channels[1], 5, norm_fn=norm_fn, padding=2, indice_key='res2_2'),
            block(channels[1], channels[1], 5, norm_fn=norm_fn, padding=2, indice_key='res2_2')
        )
    ...
    def conv2_out(self, conv2):
        feature_cat = conv2.features
        indices_cat = conv2.indices
        indices_unique, _inv = torch.unique(indices_cat, dim=0, return_inverse=True)
        feature_unique = feature_cat.new_zeros((indices_unique.shape[0], feature_cat.shape[1]))
        feature_unique.index_add_(0, _inv, feature_cat)
        conv2.indices = indices_unique
        return replace_feature(conv2, feature_unique)
    ...
    def forward(self, batch_dict):
        ...
        x_conv2_2 = self.conv2_2(x_conv1)
        x_conv2 = x_conv2.replace_feature(torch.cat([x_conv2.features, x_conv2_2.features]))
        x_conv2.indices = torch.cat([x_conv2.indices, x_conv2_2.indices])
        x_conv2 = self.conv2_out(x_conv2)

...
class VoxelResBackBone8xVoxelNeXt2DSPS(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.conv1 = SparseSequentialBatchdict(
            SparseBasicBlock(tmp_channel, tmp_channel, norm_fn=norm_fn, indice_key='res1'),
            SparseBasicBlock(tmp_channel, tmp_channel, norm_fn=norm_fn, indice_key='res1'),
        )

        self.conv2 = SparseSequentialBatchdict(
            # [1600, 1408] <- [800, 704]
            block(tmp_channel, 64, spconv_kernel_sizes[0], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[0]//2), indice_key='spconv2', conv_type=self.downsample_type[0], pruning_ratio=self.downsample_pruning_ratio[0]),

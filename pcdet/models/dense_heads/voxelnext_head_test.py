        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                fc_list.append(spconv.SparseSequential(
                    spconv.SubMConv2d(input_channels, input_channels, kernel_size, padding=int(kernel_size // 2),
                                      bias=use_bias, indice_key=cur_name),
                    nn.BatchNorm1d(input_channels),
                    nn.ReLU()
                ))
            fc_list.append(
                spconv.SubMConv2d(input_channels, output_channels, 1, bias=True, indice_key=cur_name + 'out'))
...
class VoxelNeXtHeadTest(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        # self.visualize_grad = model_cfg.VISUALIZE_GRAD
        ...
        # cur_class_names 代表需要检测的类别， 决定heatmap数量
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            for bs_idx in range(batch_size):
                ...
                for idx, name in enumerate(gt_class_names):
                    # 只检测要检测的类别， 要检测的类别在cur_class_names中
                    if name not in cur_class_names:
                        continue
                    # 更改gt_box最后面的类别索引（从all_names转换到cur_class_names下）
                ...
                # 前面所做的一切就是将gt_boxes从 all-names 转换到 cur-class-names 下，保存在gt_boxes_single_head
    ...
    def assign_target_of_single_head(
            self, num_classes, gt_boxes, num_voxels, spatial_indices, spatial_shape, feature_map_stride,
            num_max_objs=500,
        ...
        """
        # 这里将heatmap变为了和体素数量一样长度的矩阵了
        heatmap = gt_boxes.new_zeros(num_classes, num_voxels)
        # new_zeros()函数的默认的类型和设备是与gt_boxes一致的
        ...
        coord_x = torch.clamp(coord_x, min=0,
                              max=spatial_shape[1] - 0.5)  # bugfixed: 1e-6 does not work for center.int()
        coord_y = torch.clamp(coord_y, min=0, max=spatial_shape[0] - 0.5)  #
        # 相当于计算gt_boxes中点在最后的特征图上的位置
        center = torch.cat((coord_x[:, None], coord_y[:, None]), dim=-1)
        center_int = center.int()
        center_int_float = center_int.float()
        # 相当于计算gt_boxes长宽高在最后的特征图上的长宽高
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            # spatial_indices 是体素的坐标
            distance = self.distance(spatial_indices, center[k])
            inds[k] = distance.argmin()
            mask[k] = 1
            tmp1, tmp2 = center[k].int(), spatial_indices[inds[k]]
            mem_dis.append((abs(tmp1[0] - tmp2[0]) ** 2 + abs(
                tmp1[1] - tmp2[1]) ** 2).item())

            if 'gt_center' in self.gaussian_type:
                centernet_utils.draw_gaussian_to_heatmap_voxels(heatmap[cur_class_id], distance,
                                                                radius[k].item() * self.gaussian_ratio)

            if 'nearst' in self.gaussian_type:
                centernet_utils.draw_gaussian_to_heatmap_voxels(heatmap[cur_class_id], self.distance(spatial_indices,
                                                                                                     spatial_indices[
                                                                                                         inds[k]]),
                                                                radius[k].item() * self.gaussian_ratio)
        ...
        if self.training:
            with open("/ai/voxelnext_dfd_dis.txt", "a+") as f:
                for dis in mem_dis:
                    f.write(str(dis) + '\n')
    ...
    def get_loss(self):
        ...
        if self.mask_loss:
            loss += self.mask_loss
            tb_dict["mask_loss"] = self.mask_loss
        return loss, tb_dict
    ...
    def _get_predicted_boxes(self, pred_dict, spatial_indices):
        ...
        xs = (spatial_indices[:, 1:2] + center[:, 0:1]) * self.feature_map_stride * self.voxel_size[0] + \
             self.point_cloud_range[0]
        ys = (spatial_indices[:, 0:1] + center[:, 1:2]) * self.feature_map_stride * self.voxel_size[1] + \
    ...
    def rotate_class_specific_nms_iou(self, boxes, scores, iou_preds, labels, rectifier, nms_configs):
        ...
        for cls in range(self.num_class):
            ...
            scores_cls = torch.pow(scores[mask], 1 - rectifier[cls]) * torch.pow(iou_preds[mask].squeeze(-1),
                                                                                 rectifier[cls])
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts, voxel_indices, spatial_shape):
        ...
        for k in range(batch_size):
            ...
            if self.iou_branch:
                ...
                pred_boxes, pred_scores, pred_labels = self.rotate_class_specific_nms_iou(pred_boxes, pred_scores,
                                                                                          pred_ious, pred_labels,
                                                                                          self.rectifier,
                                                                                          self.nms_configs)
    ...
    def forward(self, data_dict):
        ...
        """
            batch_index: 每个体素对应的batch
            voxel_indices: 每个体素的坐标
            spatial_indices: 将每个体素分批后，进行了裁剪和换位 后得到的坐标
            num_voxels: 每个批量的体素的数量
        """
